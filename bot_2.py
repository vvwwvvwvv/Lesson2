...
def main():
    ...
    dp = mybot.dispatcher
    dp.add_handler(CommandHandler(commands, processing_com))
    mybot.start_polling()
    mybot.idle()
...
def processing_com(bot,update):
    ...
    if command[0] == "/planet" and len(command) < 2:
        ...
        try:
            full_name = ephem.constellation(ephem_planets.get(planet_name)(datetime.date.today()))
